...
exports.handleLogin = asyncHandler((req, res) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    const userId = user === null || user === void 0 ? void 0 : user.trim().toLowerCase();
    const { name: devName, os: devOs, version: devVersion } = deviceInfo;
    if (isEmail) {
        existingUser = yield userSchema_1.default.findOne({ 'mail.email': userId });
    }
    else {
        existingUser = yield userSchema_1.default.findOne({ username: userId });
    }
    ...
    const username = yield existingUser.username;
    const { name: exDevName, os: exDevOs, version: exDevVersion } = yield existingUser.deviceInfo;
    ...
    if (devName !== exDevName || devOs !== exDevOs || devVersion !== exDevVersion) {
        yield (0, mailer_1.default)('Kawojue Raheem', existingUser.mail.email, 'Login Notification', text);
    }
